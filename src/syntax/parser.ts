...
const semantics = grammar
  .createSemantics()
  .addOperation<
    FileAST | EntryAST | Temporal.PlainDate | Temporal.PlainTime | TimeRangeAST | number | string
  >("ast", {
    ...
    Year(d1, d2, d3, d4) {
      return parseInt(d1.sourceString + d2.sourceString + d3.sourceString + d4.sourceString)
    },
    ...
    Time(hour, _colon, minute) {
      return new Temporal.PlainTime(hour.ast() as number, minute.ast() as number)
    },
